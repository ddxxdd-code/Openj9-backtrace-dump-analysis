...
def build_translation_table(executable_file_path, offset_list, offset_translation_dict):
    ...
    processed_offset_list = [str(hex(int(_, base=16)))[2:] for _ in offset_list]
    args += processed_offset_list
    ...
    for i in range(len(offset_list)):
        ...
        offset_translation_dict[offset_list[i]] = [output[2*i], line + ":" + offset_list[i]]
...
def read_offsets(file):
    ...
    while line:
        if line[0:11] == "Compilation":
            continue    # skip lines with header of compilation
        elif line[0].isalpha():
